...
```{r}
Tcells[[]] <- PercentageFeatureSet(object = Tcells,
                                                 pattern = "")
...
Tcells <- subset(x = Tcells,
                 subset = nFeature_RNA > ___ & nCount_RNA > ___ & percent.mito < ___)
...
Tcells@meta.data <- Tcells@meta.data %>%
  mutate(Age = case_when(grepl("___", Sample) ~ '___',
                         grepl('___', Sample) ~ '___',
                         grepl('___', Sample) ~ '___'))
...
LabelPoints(plot = VariableFeaturePlot(Tcells), 
            points = head(VariableFeatures(Tcells), ___),
            repel = TRUE)
```
